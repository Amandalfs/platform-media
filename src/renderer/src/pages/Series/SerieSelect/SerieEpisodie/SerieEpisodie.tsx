...
import { GetSerieEpisodieByNumberResponse } from '~/src/shared/types/ipc-types'
...
export function SerieEpisodie(): JSX.Element {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['serie-episodie', id, season, episodie],
    queryFn: async (): Promise<GetSerieEpisodieByNumberResponse | void> => {
      if (id && season && episodie) {
        const response = await window.api.getSerieEpisodie({
          id,
          season,
          number: episodie,
        })
